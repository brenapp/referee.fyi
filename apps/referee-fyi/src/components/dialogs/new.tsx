...
import { type IncidentOutcome, packIncident } from "~utils/data/incident";
import { queryClient } from "~utils/data/query";
import type { UseNewIncidentDialogState } from "~utils/dialogs/new";
import { useSaveAssets } from "~utils/hooks/assets";
import { useAddRecentRules, useRecentRules } from "~utils/hooks/history";
import { type Rule, useRulesForEvent } from "~utils/hooks/rules";
	...
	const teamMatches = useMemo(() => {
		...
		return (
			matches?.filter((match) => {
				const teams = match.alliances.flatMap((a) =>
					a.teams.map((t) => t.team?.name),
				);
