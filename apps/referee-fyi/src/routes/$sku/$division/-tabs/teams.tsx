		...
		>
			...
			<div className="absolute right-0 bg-zinc-800 h-full w-32 px-2 flex items-center justify-between">
				<span className="text-red-400 mr-4">
					...
					<span className="font-mono ml-2">{majorCount}</span>
				</span>
				<span className="text-yellow-400">
					<ExclamationTriangleIcon height={20} className="inline" />
					<span className="font-mono ml-2">{minorCount}</span>
				</span>
			</div>
		</MenuButton>
	);
...
export const EventTeamsTab: React.FC<EventTagProps> = ({ event }) => {
	...
	const minorIncidents = useMemo(() => {
		...
		for (const incident of incidents) {
			if (
				incident.outcome !== "Minor" &&
				incident.outcome !== "Disabled" &&
				incident.outcome !== "General"
			)
				continue;
			const key = incident.team ?? "<none>";
			const count = grouped.get(key) ?? 0;
			...
			>
				{(team) => (
					<TeamListItem
						...
						sku={event.sku}
						incidents={incidents ?? []}
						majorCount={majorIncidents.get(team.number) ?? 0}
						minorCount={minorIncidents.get(team.number) ?? 0}
